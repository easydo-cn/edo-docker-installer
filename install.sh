...
function Update_Sh(){
	...
	sh_new_version=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/easydo-cn/edo-docker-installer/master/install.sh"|grep 'sh_version="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	...
	if [[ ! ${sh_version} == ${sh_new_version} ]]; then
		...
		[[ -z ${yn} ]] && yn="y"
		if [[ ${yn} == [Yy] ]]; then
			 wget --no-check-certificate -N https://raw.githubusercontent.com/easydo-cn/edo-docker-installer/master/install.sh && chmod u+x *.sh
		else 
		 echo -e "${Tip}取消更新!"
		fi
